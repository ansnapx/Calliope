...
ULONG CConfigKey::Crc32(char const* buffer, ULONG bufferSize)
{
...
BOOL  CConfigKey::CreateKey(char* buffer)
{	
	BOOL bRet = FALSE;
	if (buffer)
	{
		
		ZeroMemory(buffer,4*1024);
		FILFILE_HEADER_BLOCK *const block = (FILFILE_HEADER_BLOCK*) buffer;
		
		block->Magic=FILF_POOL_TAG;
		block->Version=1;
		WCHAR* pWChar=(WCHAR*)(&(block->Cipher));//= FILFILE_CIPHER_MODE_MASK;
		pWChar[0] |=FILFILE_CIPHER_SYM_AES256;
		pWChar[1] |=FILFILE_CIPHER_MODE_MASK;
		block->PayloadSize=0x312;
		block->BlockSize=0x1000;
		block->Deepness=0xffffffff;
		block->Reserved=0x329f1871;
		block->Nonce.LowPart=0xf966a724;
		block->Nonce.HighPart=0xa21e1398;
		UCHAR key[32]={0xeb,0x7e,0xc5,0xfb,0x55,0x26,0x67,0x85,0x41,0x8e,0x4f,0x22,0x83,0x4f,0x8a,0x06,0x4e,0x53,
			0xdd,0x03,0xb4,0x41,0xb0,0xa9,0xc8,0xa8,0x48,0x37,0xa0,0xb7,0x2a,0xd0};
		for (int nI=0;nI<32;nI++)
		{
			block->FileKey[nI]=key[nI];
		}
		//UCHAR palyload[786]={0};
		UCHAR* pLoad=(UCHAR*)(buffer+sizeof(FILFILE_HEADER_BLOCK));
		for (int nI=0;nI<786;nI++)
		{
			//palyload[nI]=;
			pLoad[nI]=MAX_PATH*rand()/RAND_MAX;
		}
		
		// compute CRC of Header payload
		int blockSize=sizeof(FILFILE_HEADER_BLOCK);
		ULONG const crc = Crc32(buffer + blockSize, block->PayloadSize);
